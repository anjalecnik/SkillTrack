import { Body, Controller, Delete, Get, HttpCode, HttpException, HttpStatus, Param, ParseIntPipe, Post, Put, Query, UseGuards } from "@nestjs/common"
import {
	ApiBearerAuth,
	ApiBody,
	ApiBodyOptions,
...
import { UserGuard, UserSelfGuard, UserSelfOrManagerGuard } from "src/utils/guards"
import { ROUTE_USER_HUB, ROUTE_USER, ROUTE_ACTIVITY, API_TAG_ACTIVITY, ROUTE_REQUEST, ROUTE_REVIEW } from "src/utils/constants"
...
@Controller(`/${ROUTE_USER_HUB}/${ROUTE_USER}/:userId/${ROUTE_ACTIVITY}`)
@ApiTags(API_TAG_ACTIVITY)
@ApiBearerAuth()
@UseGuards(UserGuard())
export class UserHubActivityController {
	...
	@UseGuards(UserSelfOrManagerGuard)
	@ApiOperation({ summary: "Return last daily activity from user (Also returns partial activity if only working hours are reported)" })
	...
	@UseGuards(UserSelfOrManagerGuard)
	@ApiOperation({ summary: "Return workspace user activity by id" })
	...
	@Get(`${ROUTE_REQUEST}`)
	@UseGuards(UserSelfOrManagerGuard)
	@ApiOperation({ summary: "Return workspace user activities" })
	@ApiOkResponse({ description: "Returns activities list", type: UserActivityListHalResponse })
	...
	async createActivity(
		...
		const activityResponse = await this.userActivityService.handleCreateActivity({ ...authPassport, requestOriginHub: originHub.userHub }, activity, {
			userId,
	...
	@UseGuards(UserSelfOrManagerGuard)
	@ApiOperation({ summary: "Updates user activity" })
	...
	async updateActivity(
		...
		const activityResponse = await this.userActivityService.handleUpdateActivity({ ...authPassport, requestOriginHub: originHub.userHub }, activity, {
	...
	async cancelActivity(
		...
		await this.userActivityService.handleCancelActivity(
			{ ...authPassport, requestOriginHub: originHub.userHub },
			{
				reportedByUserId: authPassport.user.id,
				userId,
				id: activityRequestId
	...
	@ApiOperation({ summary: "Review workspace user activity" })
	@ApiOkResponse(SwaggerGetActivityRequestSchema)
